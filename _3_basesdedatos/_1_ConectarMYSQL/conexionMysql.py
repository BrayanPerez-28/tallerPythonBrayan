...
class ConexionMysql:
    ...
    def Insertar(self, tabla, valores)->bool:
        sql = f"INSERT INTO {tabla} (id, nombre-completo, nombre_usuario, correo_electronico, contrasennia, id_rol) VALUES (%s, %s, %s, %s, %s, %s)"
...
if __name__ == '__main__':
    conexion=ConexionMysql("localhost",3306,"almacen","root","")
    ...
    if conexion.BanConexion:
        # if conexion.Insertar("datos",("Cesar", "Contreras Polanco", 41)):
        ...
        if res != None:
            for fila in res:
                print(f" ID: {fila[0]}, Nombre Completo: {fila[1]}, Nombre de usuario: {fila[2]}, Coreo electronico: {fila[3]}, Contraseña: {fila[4]}, ID_Rol{fila[5]}")
        else:
